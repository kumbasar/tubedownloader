...
parser.add_argument("-s", "--skip", help = "Skip existing", type = bool, required= False, default=False)
...
yt.streams.filter(file_extension='mp4',type="video").order_by('resolution')[-1].download( output_path='tmp', filename='video', skip_existing=args.force)

print("Downloading audio part:")
print(yt.streams.get_audio_only("mp4"))
yt.streams.get_audio_only("mp4").download(output_path='tmp', filename='audio', skip_existing=args.force)
